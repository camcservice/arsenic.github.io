                ...
                if ($clientFilesHeader) {
                    ...
                    foreach ($filesToExclude as $fileToExclude) {
                        ...
                        if ($zip->deleteName($pathInZip) !== TRUE) {
                            $zip->close();
                            @unlink($cachedAppPath);
                            $this->logger->error("Failed to delete file $fileToExclude from package zip file");
                            $response = new CSProResponse();
                            $response->setError(500, 'app_download_failed', "Failed to delete file $fileToExclude from package zip file ");
                            return $response;
                        }
                    }
                    $zip->close();
