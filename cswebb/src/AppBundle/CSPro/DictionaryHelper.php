...
EOT;
        ...
        foreach ($levelOneIds as $idItem) {
            $sql .= $this->generateItemSql($idItem, false);
        }
...
EOT;
    ...
    function updateExistingDictionary($dict, $dictContent, &$csproResponse) {
        ...
        try {
            ...
            $idcols = $this->pdo->fetchAll("SHOW COLUMNS FROM `{$dictName}`LIKE 'id_%'");
            $levelOneIds = $dict->getLevels()[0]->getIdItems();
            ...
            function columnMatchesId($col, $id) {
                ...
                if (!preg_match("/^(int|bigint|char)\((\d*)\)$/i", $col['Type'], $matches))
                    return false;
            ...
            $missingFromTable = array_filter($levelOneIds, function ($i) use($idcols) {
                return empty(array_filter($idcols, function ($c) use($i) {
                            return columnMatchesId($c, $i);
                        }));
            });
            if (!empty($missingFromTable)) {
                $missingNames = implode(', ', array_map(function ($i) {
                            return $i->getName();
                        }, $missingFromTable));
                $this->logger->error("Failed to update dictionary: Id items missing from existing table $missingNames");
            ...
            $missingFromDict = array_filter($idcols, function ($c) use($levelOneIds) {
                return empty(array_filter($levelOneIds, function ($i) use($c) {
                            return columnMatchesId($c, $i);
                        }));
            });
            if (!empty($missingFromDict)) {
                $missingNames = implode(', ', array_map(function ($c) {
                            return $c['Field'];
                        }, $missingFromDict));
                $this->logger->error("Failed to update dictionary: Id items missing from dictionary $missingNames");
                $csproResponse->setError(400, 'dictionary_changed', "Id items do not match existing data ($missingNames). Download and reformat existing data to match the new dictionary. Then delete existing data on the server and replace it with the reformatted data");
                $csproResponse->setStatusCode(400);
                return;
            }
    ...
    function getCaseIdColumnValues($dict, $questionnaire) {
        $levelOneIds = $dict->getLevels() [0]->getIdItems();
        $dataParser = new \AppBundle\CSPro\Data\Parser($dict, $questionnaire);
